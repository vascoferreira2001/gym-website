<?php
// Página de edição de dados do Cliente
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE users SET name=?, idade=?, email=?, telemovel=?, morada=? WHERE id=? AND role='cliente'");
...
// Buscar dados atuais
$stmt = $conn->prepare("SELECT name, idade, email, telemovel, morada FROM users WHERE id=? AND role='cliente'");
...
<div class="container mt-5">
    <h2 class="fw-bold mb-4" style="color:#ff6633;">Editar Cliente</h2>
    <?= $msg ?>
    <form method="post" class="mb-4">
        ...
        <a href="ver_clientes.php" class="btn btn-secondary">Voltar</a>
    </form>
</div>
